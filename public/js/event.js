define("ZSYWEB.Event", [], function (){
    ...
    EventCollector.delegate = function(pDom, dom, evType, fnName, handle, scop) {
        ...
        if (handle[fnName] != undefined) {
            EMG.registerEvent(fnName, fn, scop)
        }
        if (handle[fnName + ":before"] != undefined) {
            fn = handle[fnName + ":before"];
            EMG.addBeforeInterceptor(fnName, fn, scop)
        } else if (handle[fnName + ":after"] != undefined) {
            fn = handle[fnName + ":after"];
            EMG.addAfterInterceptor(fnName, fn, scop)
        }
        if(evType == "swipe") {
            Events.bind($(pDom), dom, evType, fn);
        }else{
            Events.bind($(pDom), dom, evType, function( currentDom, evt ){
                EMG.invoke(fnName, {
                     ev: evt,
                     current: currentDom
                },scop);
    ...
    NameSpace.Register("ZSYWEB.Event", Events);
    NameSpace.Register("ZSYWEB.EventCollector", EventCollector);
    return Events;
});