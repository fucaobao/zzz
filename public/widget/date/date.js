 ...
 */
define("ZSYWEB.Widget.Date!CSS", ['ZSYWEB.Comm', 'ZSYWEB.Event','ZSYWEB.NativeAPI'], function () {
    var a = window;
    var b = {
        path: "",
        defSkin: "date",
    ...
    h = ["laydate_box", "laydate_void", "laydate_click", "LayDateSkin"];
    window.laydate = function(b) {
        b = b || {};
        try {
            window.event = window.event || laydate.caller.arguments[0]
        } catch(d) {}
        return window.event && (b.tagName = 1),
        c.run(b),
        laydate
    },
    laydate.v = "1.1",
    ...
    console.info(window.laydate);
    NameSpace.Register("ZSYWEB.Widget.Date", a);
    return a;
});